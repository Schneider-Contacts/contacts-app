...
/**
 * תאימות לאחור בלבד: מנגנון הכניסה אינו מבצע עוד lookup מנהלי ב-Firebase Auth.
 * הפונקציה אינה משנה נתונים ואינה דורשת API או הרשאת OAuth נוספת.
 */
function diagnoseFirebaseAuthLookup() {
  const result = {
    ok: true,
    message:
      "בדיקת Firebase Auth מוקדמת אינה נדרשת עוד. " +
      "מסלול הכניסה בודק רק הרשאה ב-allowedUsers ולאחר הזנת סיסמה " +
      "Firebase מחליט אם להתחבר או ליצור חשבון חדש."
  };
  Logger.log(JSON.stringify(result, null, 2));
  return result;
}
...
function getPasswordResetRequest_(email) {
  ...
  if (response.getResponseCode() === 404) return null;
  if (response.getResponseCode() < 200 || response.getResponseCode() >= 300) {
    throw new Error("קריאת בקשת איפוס הסיסמה נכשלה.");
  }
...
function savePasswordResetRequest_(email) {
  ...
  const url =
    ...
    "&updateMask.fieldPaths=requestedAt" +
    "&updateMask.fieldPaths=updatedAt" +
    "&updateMask.fieldPaths=handledAt" +
    "&updateMask.fieldPaths=handledBy" +
    "&updateMask.fieldPaths=sentAt";
  ...
  if (response.getResponseCode() < 200 || response.getResponseCode() >= 300) {
    throw new Error("שמירת בקשת איפוס הסיסמה נכשלה.");
  }
}
...
function createPasswordResetRequestJsonp_(e) {
  ...
  try {
    ...
    const cache = CacheService.getScriptCache();
    const cooldownKey = getPublicAuthRouteCacheKey_(
      "password-reset",
      email
    );
    const cachedPayload = cache.get(cooldownKey);
    ...
    if (cachedPayload) {
      ...
      if (!eligible) {
        ...
        if (existing && existing.status === "pending") {
          ...
          savePasswordResetRequest_(email);
          payload = {
            ok: true,
            duplicate: false,
            managerName: supportContact && supportContact.ok
      ...
      cache.put(
        ...
        PASSWORD_RESET_PUBLIC_COOLDOWN_SECONDS
      );
    }
  } catch (error) {
    console.error("Password reset request failed:", error);
...
function verifyFirebaseAdminIdToken_(idToken) {
  const token = cleanSheetValue_(idToken);
  if (!token || token.length > 5000) {
    throw new Error("לא התקבל אימות מנהל תקין.");
  }
  ...
  if (
    ...
    throw new Error("אימות המנהל פג. יש להתחבר מחדש ולנסות שוב.");
  }
