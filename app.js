  ...
  if (authPurpose === "guided") {
    ...
    if (modeNote) {
      ...
      modeNote.textContent =
        "בכניסה ראשונה החשבון ייווצר עם הסיסמה שהקלדת ורק לאחר מכן יישלח מייל אימות.";
    }
    return;
  }
...
async function updateManagerWhatsappLink_() {
  ...
  link.textContent = "שליחת WhatsApp למנהל הפעיל";
...
function setManualApprovalRequestState_(state = "idle", message = "") {
  ...
  if (button) {
    ...
    button.textContent = normalizedState === "pending"
      ? "הבקשה נשלחה וממתינה לאישור מנהל־העל"
      : normalizedState === "approved"
        ? "הגישה אושרה על ידי מנהל־העל"
        : normalizedState === "sending"
          ? "שולח בקשה..."
          : normalizedState === "rejected"
            ? `שליחת בקשה חדשה למנהל־העל${managerSuffix}`
    ...
    if (state === "pending") {
      ...
      setManualApprovalRequestState_(
        ...
        state === "revoked"
          ...
          : "הבקשה הקודמת נדחתה. ניתן לפנות למנהל־העל ולשלוח בקשה חדשה לפי הצורך."
      );
    } else {
      setManualApprovalRequestState_("idle", "");
    }
...
function startVerificationAccessListener_(user) {
  ...
  verificationAccessListenerUnsubscribe = firebaseApi.onSnapshot(
    ...
    snapshot => {
      ...
      if (data.manualApproved !== true || verificationAccessTransitionInProgress) return;
      if (!auth || !auth.currentUser || normalizeEmail(auth.currentUser.email) !== email) return;

      verificationAccessTransitionInProgress = true;
      setManualApprovalRequestState_(
        "approved",
        "הגישה אושרה על ידי מנהל־העל. מכניסים אותך לאפליקציה..."
      );
...
async function requestManualApproval_() {
  ...
  setManualApprovalRequestState_("sending", "שולח בקשה למנהל־העל...");
...
async function handleAuthenticatedUser(user, options = {}) {
  ...
  try {
    ...
    if (user.emailVerified) {
      ...
      // משתמש שלא אימת מייל יכול להיכנס רק לאחר אישור ידני של מנהל־על.
    ...
    recordOwnAuthState_(user.emailVerified ? "verified" : "manual_approved");
...
async function loadAdminUsersData_() {
  ...
  adminVerificationRequests = verificationRequestsSnapshot.docs.map(document => {
    ...
    return {
      docId: document.id,
      email: normalizeEmail(data.email || document.id),
      status: ["pending", "approved", "rejected", "revoked"].includes(data.status)
        ? data.status
        : "pending",
  ...
  adminPasswordResetRequests = passwordResetRequestsSnapshot.docs.map(document => {
    ...
    return {
      docId: document.id,
      email: normalizeEmail(data.email || document.id),
      status: ["pending", "sent", "closed"].includes(data.status)
        ? data.status
        : "pending",
      requestedAt: data.requestedAt || null,
      updatedAt: data.updatedAt || null,
...
async function approveManualAccess_(email) {
  if (!currentUserIsSuperAdmin) {
    alert("רק מנהל־על יכול לאשר גישה ללא אימות מייל.");
    return;
  }
  ...
  if (!user || !user.active || !request || request.status !== "pending") {
  ...
  setAdminStatus("מאשר גישה ידנית...", "loading");
  try {
    const batch = firebaseApi.writeBatch(db);
    const now = firebaseApi.serverTimestamp();
    batch.set(
    ...
    await batch.commit();
    await loadAdminData();
    setAdminStatus("הגישה אושרה ידנית. המשתמש יוכל להיכנס עם הסיסמה שקבע.", "success");
  } catch (error) {
...
function renderAdminUsers() {
  ...
  const users = adminAllowedUsers
    .filter(user => {
      ...
      if (adminActiveFilter === "pending" && accessState.key !== "pending") return false;
  ...
  const pendingCount = adminAllowedUsers.filter(user => getUserAccessState_(user).key === "pending").length;
  ...
  document.getElementById("adminSummary").textContent =
    ...
    (pendingCount ? ` · ${pendingCount} בקשות אימות ממתינות` : "") +
  ...
  const resetRequestHtml = resetRequests.map(request => {
    ...
    return `
      <div class="adminCard reportCard">
        ...
        <div class="adminCardActions">
          <button type="button" class="adminActionBtn primary" onclick="sendPasswordResetForUser_('${escapeJsString(request.email)}')">שליחת קישור איפוס</button>
          <button type="button" class="adminActionBtn secondary" onclick="closePasswordResetRequest_('${escapeJsString(request.email)}')">סגירה ללא שליחה</button>
        </div>
      </div>
    `;
  }).join("");
  ...
  const usersHtml = visibleUsers.map(user => {
    ...
    if (accessState.key === "pending" && request && request.status === "pending") {
      approvalActions = currentUserIsSuperAdmin
        ? `<button type="button" class="adminActionBtn primary" onclick="approveManualAccess_('${escapeJsString(user.email)}')">אישור גישה ידני</button>
           <button type="button" class="adminActionBtn warning" onclick="rejectManualAccess_('${escapeJsString(user.email)}')">דחיית בקשה</button>`
        : '<button type="button" class="adminActionBtn secondary" disabled>ממתין לאישור מנהל־על</button>';
    } else if (user.manualApproved && currentUserIsSuperAdmin) {
